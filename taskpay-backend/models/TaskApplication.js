module.exports = (sequelize, DataTypes) => {
    class TaskApplication extends sequelize.Sequelize.Model {
        static associate(models) {
            // TaskApplication belongs to a User (Applicant)
            TaskApplication.belongsTo(models.User, {
                foreignKey: 'Applicant_ID',
                as: 'ApplicantDetails'
            });

            // TaskApplication belongs to a Task
            TaskApplication.belongsTo(models.Task, {
                foreignKey: 'Task_ID',
                as: 'TaskDetails'
            });
        }
    ...
    TaskApplication.init({
        ...
        Applicant_ID: {
            ...
            references: { model: 'Users', key: 'UserID' } // Table name
        },
        Task_ID: {
            type: DataTypes.INTEGER, allowNull: false,
            references: { model: 'Tasks', key: 'TaskID' } // Table name
        },
        ApplicationDate: { type: DataTypes.DATE, defaultValue: DataTypes.NOW },
        Status: {
            type: DataTypes.ENUM(
                'Pending', 'ViewedByAdmin', 'Shortlisted', 'Approved',
                'Withdrawn', 'Rejected', 'InProgress',
                'SubmittedForReview', 'Completed', 'Cancelled'
            ),
